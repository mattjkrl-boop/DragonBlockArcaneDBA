...
public class RaceLoader extends SimplePreparableReloadListener<Map<Identifier, JsonElement>> {

    @Override
    protected Map<Identifier, JsonElement> prepare(ResourceManager resourceManager, ProfilerFiller profiler) {
        Map<Identifier, JsonElement> prepared = new HashMap<>();
        FileToIdConverter converter = FileToIdConverter.json("races");
        ...
        resources.forEach((id, resource) -> {
            try (Reader reader = resource.openAsReader()) {
                ...
                System.err.println("Failed to read race JSON: " + id + " - " + e.getMessage());
            }
        });
        ...
        prepared.forEach((id, element) -> {
            try {
                if (element.isJsonObject()) {
                    ...
                    DbaRegistries.registerRace(race);
                }
            } catch (Exception e) {
                System.err.println("Failed to load race " + id + ": " + e.getMessage());
            }
        });
    }
}
