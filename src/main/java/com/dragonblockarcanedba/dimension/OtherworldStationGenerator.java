...
public class OtherworldStationGenerator {
    ...
    public static Vec3 ensureStationAndGetSpawn(ServerLevel level) {
        ...
        if (existingCenter != null) {
            ...
            // Find optimal surface Y from the generated terrain
            int maxGround = 64;
            for (int x = -6; x <= 6; x += 3) {
            ...
            int baseY = Math.max(70, Math.min(180, maxGround + 1));
        ...
        // 3. Ensure the Otherworld Guide is alive at the desk
        ensureGuide(level, stationCenter);

        // 4. Return safe spawn location in front of King Yemma's desk
        return new Vec3(stationCenter.getX() + 0.5, stationCenter.getY(), stationCenter.getZ() - 2.5);
    }

    /**
     * Checks if the station was previously generated by scanning for the central golden floor tile.
     */
    private static BlockPos findExistingStation(ServerLevel level) {
        for (int y = 50; y <= 200; y++) {
            BlockPos p = new BlockPos(0, y - 1, 0);
            if (level.getBlockState(p).is(Blocks.GOLD_BLOCK)
                    && level.getBlockState(p.above()).is(Blocks.AIR)
                    && level.getBlockState(p.offset(0, 1, 3)).is(Blocks.LECTERN)) {
