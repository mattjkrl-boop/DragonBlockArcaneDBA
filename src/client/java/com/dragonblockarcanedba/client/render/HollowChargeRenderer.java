...
/**
 * Entity Renderer for Hollow Rush Charge-up in Minecraft 26.2.
 * Renders physical 3D ground containment runes, orbiting inward-converging void shards, a pulsating singularity octahedron matrix, and ascending helical shadow vortex ribbons.
 */
public class HollowChargeRenderer extends EntityRenderer<HollowChargeEntity, HollowChargeRenderer.ChargeRenderState> {

    public HollowChargeRenderer(EntityRendererProvider.Context context) {
        super(context);
    }
    ...
    @Override
    public boolean shouldRender(HollowChargeEntity entity, net.minecraft.client.renderer.culling.Frustum frustum, double cameraX, double cameraY, double cameraZ) {
        return true;
    }
    ...
    @Override
    public void extractRenderState(HollowChargeEntity entity, ChargeRenderState state, float partialTicks) {
    ...
    public void submit(ChargeRenderState state, PoseStack poseStack, SubmitNodeCollector collector, CameraRenderState cameraState) {
        ...
        float baseRadius = 1.4f + (charge * 1.6f);
        float baseAlpha = 0.60f + (charge * 0.35f);
        float pulse = 0.88f + 0.12f * (float) Math.sin(age * 0.45f);

        collector.submitCustomGeometry(poseStack, renderType, (pose, buffer) -> {
            Matrix4f matrix = pose.pose();

            // 1. Planar 3D Ground Spatial Containment Runes (3 counter-rotating ground rings)
            int groundSegments = 28;
            // Outer Ring: Deep Void Violet
            drawRotatingRing(matrix, buffer, 0, 0.03f, 0, baseRadius * 1.15f, baseRadius * 0.98f, groundSegments, age * -14.0f,
                0.35f, 0.02f, 0.65f, baseAlpha * 0.75f);
            // Middle Ring: Vibrant Purple-to-Cyan Accretion
            drawRotatingRing(matrix, buffer, 0, 0.06f, 0, baseRadius * 0.85f, baseRadius * 0.72f, groundSegments, age * 24.0f,
                0.65f, 0.05f, 0.95f, baseAlpha * 0.85f);
            // Inner Core Ring: Bright Neon Cyan
            drawRotatingRing(matrix, buffer, 0, 0.09f, 0, baseRadius * 0.55f, baseRadius * 0.42f, groundSegments, age * -36.0f,
                0.15f, 0.90f, 1.0f, baseAlpha * 0.95f);

            // 2. Inward-Converging Gravitational Void Shards (3D dark-matter crystalline pyramids)
            int shardCount = 8 + (int) (charge * 6); // 8 to 14 shards
            float orbitRadius = (2.2f - charge * 0.7f) * pulse;
            float orbitSpeed = 18.0f + (charge * 50.0f);

            for (int i = 0; i < shardCount; i++) {
                double shardAngle = (i / (double) shardCount) * Math.PI * 2.0 + Math.toRadians(age * orbitSpeed);
                float sy = 0.4f + (float) Math.sin(age * 0.3f + i * 1.2f) * 0.5f + (i % 2 == 0 ? 0.3f : 0.0f);

                float sx = (float) Math.cos(shardAngle) * orbitRadius;
                float sz = (float) Math.sin(shardAngle) * orbitRadius;

                // Angle tip directly inward toward center (y = 0.9)
                float targetY = 0.9f;
                float toCenterX = -sx;
                float toCenterZ = -sz;
                ...
                draw3DShard(matrix, buffer,
                    ...
                    0.20f, 0.0f, 0.40f, baseAlpha * 0.85f,
                    0.15f, 0.95f, 1.0f, baseAlpha * 0.98f
                );
            }

            // 3. Singularity Polyhedron Matrix (Pulsating nested 3D octahedrons around player)
            float coreSize = (0.75f - charge * 0.25f) * pulse;
            drawSingularityOctahedron(matrix, buffer, coreSize, 0.85f, age * 20.0f, 0.08f, 0.0f, 0.15f, baseAlpha * 0.80f);
            drawSingularityOctahedron(matrix, buffer, coreSize * 0.55f, 0.85f, -age * 30.0f, 0.70f, 0.10f, 1.0f, baseAlpha * 0.95f);
            if (charge >= 0.5f) {
                drawSingularityOctahedron(matrix, buffer, coreSize * 0.25f, 0.85f, age * 50.0f, 1.0f, 1.0f, 1.0f, 1.0f);
            }

            // 4. Twin Helical Shadow Vortex Ribbons (Ascending dark ribbons)
            int ribbonSteps = 12;
            for (int strand = 0; strand < 2; strand++) {
                ...
                for (int s = 0; s < ribbonSteps; s++) {
                    ...
                    double ang1 = strandOffset + (t1 * Math.PI * 2.5) + (age * 0.18);
                    double ang2 = strandOffset + (t2 * Math.PI * 2.5) + (age * 0.18);
                    ...
                    drawQuad(matrix, buffer,
                        ...
                        x2 + rw, y2, z2 + rw,
                        x2 - rw, y2, z2 - rw,
                        0.45f, 0.05f, 0.85f, baseAlpha * (1.0f - t1 * 0.4f)
                    );
                }
            }
        });
    ...
    private static void drawSingularityOctahedron(Matrix4f matrix, VertexConsumer consumer, float s, float centerY, float rotDeg, float r, float g, float b, float a) {
