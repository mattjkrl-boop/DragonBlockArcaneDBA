...
/**
 * Entity Renderer for Brave Sword Heroic Impact Shockwave in Minecraft 26.2.
 * Renders physical 3D golden raised beveled crests, cyan harmonic compression rings,
 * vertical crystalline valor blades/spikes, and an epicenter starburst core.
 */
public class BraveShockwaveRenderer extends EntityRenderer<BraveShockwaveEntity, BraveShockwaveRenderer.ShockwaveRenderState> {

    public BraveShockwaveRenderer(EntityRendererProvider.Context context) {
        super(context);
    }
    ...
    @Override
    public boolean shouldRender(BraveShockwaveEntity entity, net.minecraft.client.renderer.culling.Frustum frustum, double cameraX, double cameraY, double cameraZ) {
        return true;
    }
    ...
    @Override
    public void extractRenderState(BraveShockwaveEntity entity, ShockwaveRenderState state, float partialTicks) {
    ...
    public void submit(ShockwaveRenderState state, PoseStack poseStack, SubmitNodeCollector collector, CameraRenderState cameraState) {
        ...
        float alpha = (1.0f - (progress * progress)) * 0.92f;
        ...
        collector.submitCustomGeometry(poseStack, renderType, (pose, buffer) -> {
            ...
            // 1. Raised Beveled 3D Heroic Golden Shockwave Ring
            int segments = 32;
            float ringWidth = Math.min(1.5f, currentRadius * 0.35f);
            float rOuter = currentRadius;
            float rInner = Math.max(0.1f, currentRadius - ringWidth);
            float crestHeight = 0.70f * (1.0f - progress * 0.75f);

            drawBeveledShockRing(matrix, buffer, 0, 0.05f, 0, rOuter, rInner, crestHeight, segments,
                1.0f, 0.84f, 0.0f, alpha,
                1.0f, 1.0f, 0.90f, alpha * 0.95f
            );

            // 2. Secondary Inner Cyan Harmonic Compression Ring
            float rSubOuter = currentRadius * 0.58f;
            float rSubInner = Math.max(0.05f, rSubOuter - (ringWidth * 0.6f));
            drawFlatRing(matrix, buffer, 0, 0.08f, 0, rSubOuter, rSubInner, 24, 0.0f, 0.95f, 1.0f, alpha * 0.70f);

            // 3. Physical Vertical Heroic Valor Blades / Crystalline Needles
            int shardCount = 18;
            float shardRadius = currentRadius * 0.85f;
            float shardHeight = 1.7f * (1.0f - progress);
            ...
            for (int i = 0; i < shardCount; i++) {
                ...
                // Outward radial lean
                float leanX = (float) Math.cos(angle) * 0.45f * (1.0f - progress);
                float leanZ = (float) Math.sin(angle) * 0.45f * (1.0f - progress);

                float shardWidth = 0.30f * (1.0f - progress * 0.5f);
                ...
                drawTriangle(matrix, buffer,
                    ...
                    sx - perpX, 0.05f, sz - perpZ,
                    sx + leanX, 0.05f + shardHeight, sz + leanZ,
                    0.0f, 0.95f, 1.0f, alpha * 0.88f
                );
            }

            // 4. Ground Epicenter Starburst Core
            float coreRadius = Math.max(0.2f, (1.0f - progress) * 2.2f);
            drawFlatRing(matrix, buffer, 0, 0.12f, 0, coreRadius, 0.0f, 16, 1.0f, 0.98f, 0.85f, alpha * 0.95f);
        });
