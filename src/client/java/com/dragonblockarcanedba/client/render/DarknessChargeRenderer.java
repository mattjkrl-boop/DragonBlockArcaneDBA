...
/**
 * Entity Renderer for Darkness Sword Abyssal Slash Charge-up in Minecraft 26.2.
 * Renders physical 3D ground demonic runes, orbiting inward-converging shadow prisms,
 * a pulsating nested void octahedron core matrix, and ascending helical dark void ribbons.
 */
public class DarknessChargeRenderer extends EntityRenderer<DarknessChargeEntity, DarknessChargeRenderer.ChargeRenderState> {

    public DarknessChargeRenderer(EntityRendererProvider.Context context) {
        super(context);
    }
    ...
    @Override
    public boolean shouldRender(DarknessChargeEntity entity, net.minecraft.client.renderer.culling.Frustum frustum, double cameraX, double cameraY, double cameraZ) {
        return true;
    }
    ...
    @Override
    public void extractRenderState(DarknessChargeEntity entity, ChargeRenderState state, float partialTicks) {
    ...
    public void submit(ChargeRenderState state, PoseStack poseStack, SubmitNodeCollector collector, CameraRenderState cameraState) {
        ...
        collector.submitCustomGeometry(poseStack, renderType, (pose, buffer) -> {
            ...
            // 1. Planar 3D Ground Demonic Seals (3 counter-rotating ground rings)
            int groundSegments = 28;
            // Outer Ring: Abyssal Obsidian & Violet Demonic Boundary
            drawRotatingRing(matrix, buffer, 0, 0.03f, 0, baseRadius * 1.15f, baseRadius * 0.98f, groundSegments, age * -14.0f,
                0.12f, 0.0f, 0.25f, baseAlpha * 0.85f);
            // Middle Ring: Corrupted Purple Void Accretion
            drawRotatingRing(matrix, buffer, 0, 0.06f, 0, baseRadius * 0.82f, baseRadius * 0.68f, groundSegments, age * 22.0f,
                0.55f, 0.05f, 0.85f, baseAlpha * 0.92f);
            // Inner Core Disc: Demonic Crimson Energy Flange
            drawRotatingRing(matrix, buffer, 0, 0.09f, 0, baseRadius * 0.50f, baseRadius * 0.36f, groundSegments, age * -32.0f,
                0.95f, 0.05f, 0.35f, baseAlpha * 0.95f);

            // 2. Inward-Converging Gravitational Shadow Prisms (3D obsidian/purple crystalline shards)
            ...
            for (int i = 0; i < shardCount; i++) {
                ...
                draw3DShard(matrix, buffer,
                    ...
                    0.08f, 0.0f, 0.18f, baseAlpha * 0.85f,
                    0.65f, 0.05f, 0.95f, baseAlpha * 0.98f
                );
            }

            // 3. Abyssal Void Octahedron Core Matrix (Pulsating nested 3D octahedrons around caster)
            float coreSize = (0.70f - charge * 0.20f) * pulse;
            drawOctahedron(matrix, buffer, coreSize, 0.85f, age * 20.0f, 0.15f, 0.0f, 0.30f, baseAlpha * 0.80f);
            drawOctahedron(matrix, buffer, coreSize * 0.58f, 0.85f, -age * 30.0f, 0.60f, 0.05f, 0.90f, baseAlpha * 0.95f);
            if (charge >= 0.4f) {
                drawOctahedron(matrix, buffer, coreSize * 0.28f, 0.85f, age * 50.0f, 0.95f, 0.15f, 0.45f, 1.0f);
            }

            // 4. Twin Helical Void Light Ribbons (Ascending dark violet and crimson ribbons)
            int ribbonSteps = 12;
            for (int strand = 0; strand < 2; strand++) {
                float strandOffset = strand * (float) Math.PI;
                boolean isVioletStrand = (strand == 0);
                float sr = isVioletStrand ? 0.45f : 0.95f;
                float sg = 0.02f;
                float sb = isVioletStrand ? 0.85f : 0.25f;
                ...
                for (int s = 0; s < ribbonSteps; s++) {
                    ...
                    double ang1 = strandOffset + (t1 * Math.PI * 2.6) + (age * 0.18);
                    double ang2 = strandOffset + (t2 * Math.PI * 2.6) + (age * 0.18);
