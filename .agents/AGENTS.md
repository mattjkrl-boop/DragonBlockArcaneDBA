...
- **Cinematic Damage Tracker System (Delayed Damage)**: To make weapon abilities feel more impactful, massive burst damage should be delayed until the weapon's visual or CC effects (lifting, rooting, pulling) on the target have completely finished. This is handled centrally by `DelayedDamageMixin.java` which caches the player's damage and waits for a 0.5-second buffer (10 ticks) before applying it. 
  - **Rule**: Whenever you add a new weapon that applies a long visual/CC status effect (like levitation, root, throw) before the main damage should hit, you MUST add that specific `MobEffect` to the `isCinematicallyLocked` check in `DelayedDamageMixin.java`. This will dynamically lock the damage combo timer for the duration of the effect, ensuring the damage burst always perfectly syncs with the end of the animation/effect for that specific mob.